...
- **最新迭代**：CR-20260708-001（订单生成历史记录规则收口 + 预占库存订单命名统一）
...
| `/product-apply/detail/:id` | 详情页 | 状态驱动首屏差异化（处理中/已驳回/已结束）；审批流节点字段化+视觉区分；预占库存订单仅在发起节点 chip 展示，订单结果区不承接预占库存订单；订单结果状态统一为"已创建/客服已审核/财务已审核/草稿"；订单生成历史按订单号/draft 聚合，默认折叠（§6.10.13）；草稿删除原因走 outerRemark 通道；失败原因去重；专卖店变更箭头样式；**场景验收样本总表**覆盖10个正式场景；Mock 双入口（`/detail/:id` + `?scene=xxx`）；详情页规则以《产品申请工单详情页面规则明细.md》（**v0.5**）为直接执行依据 |
...
- 最新迭代：CR-20260708-001（订单生成历史记录规则收口 + 预占库存订单命名统一），详见 `../docs/development-summary/2026-07-08/`
- 前序迭代：CR-20260707-003（详情页场景验收样本补强、Mock 入口整理）+ CR-20260707-002（订单结果区重试历史规则、状态术语统一）+ CR-20260707-001（详情页现行规则口径统一）+ CR-20260706-004（专卖店变更、失败原因类型）

...
| `/product-apply/detail/:id` | 详情页 | 状态驱动首屏差异化（处理中/已驳回/已结束）；审批流节点字段化+视觉区分；多笔预占订单 chip 展示；失败原因去重；折叠重试历史；区块顺序：状态→驳回/审批进度→基础→发起→预算→明细→附件→审批流/订单结果→操作；订单结果区展示实际专卖店（橙色警告）+ 订单备注（灰色斜体）；列表页滚动不带入详情页（`scrollBehavior`） |

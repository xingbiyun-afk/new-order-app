...
| `/product-apply/detail/:id` | 详情页 | 状态驱动首屏差异化（处理中/已驳回/已结束）；审批流节点字段化+视觉区分；多笔预占订单 chip 展示；失败原因去重；折叠重试历史；区块顺序：状态→驳回/审批进度→基础→发起→预算→明细→附件→审批流/订单结果→操作；订单结果区展示实际专卖店（橙色警告）+ 订单备注（灰色斜体）；失败原因类型明确（产品库存不足/专卖店下单标识关闭/订单审核驳回草稿）；Mock 11 场景覆盖（id 1-11）；移除导航栏状态字；`scrollBehavior` 修复列表→详情不继承滚动位置 |
...
- 最新迭代：CR-20260706-004（订单结果区专卖店变更、失败原因类型、备注显示），详见 `../docs/development-summary/2026-07-06/`

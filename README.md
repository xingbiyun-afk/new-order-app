...
- **已完成（2026-07-09）：CR-20260709-003 审批超时自动驳回、组织变更场景在途工单驳回及详情承接规则补增**
  - `process-and-status-rules.md` v0.1→v0.2：新增 §10 审批超时自动驳回规则（14天阈值）、§11 组织变更场景在途工单处理规则、§12 财务通知与系统驳回关系、§13 驳回原因4类分类总表
  - **新建** `产品申请审批流程管理页面规则明细.md`（v0.1）：审批人来源、超时流程边界、组织变更不重算说明、与异常监控职责边界
  - **新建** `审批人员异常监控页面规则明细.md`（v0.1）：超时监控口径、组织变更后监控口径、监控提示与审批结果边界

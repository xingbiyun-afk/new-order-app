...
### CR-20260708-001：订单生成历史记录规则收口、预占库存订单命名统一与 Mock 补强

**完成时间**：2026-07-08
**分支**：`feature/CR-20260708-001-order-generation-history-rules`

**主要变更**：
1. 订单生成历史记录规则正式收口（§6.10.13）：
   - 同一订单号在 retryHistory 中只出现一次（一单一录）
   - 审核状态不体现在生成历史中（审核≠生成）
   - 无订单号草稿提交使用 `"draft"` key 聚合
   - 删除草稿原因走 `GroupResult.outerRemark`，不入 retryHistory
   - 预占库存订单与 relatedOrders 互不干扰
2. 术语统一：全仓"预占订单"→"预占库存订单"（代码注释 + UI 标签 + 文档共 20+ 处）
3. 类型定义更新：`RelatedOrder.orderNo` 改为可选；`GroupResult` 新增 `outerRemark?`
4. Mock 场景 8/9 数据重构，对齐 §6.10.13 新规则（draft key、outerRemark、无订单号记录）
5. UI 适配：relatedOrders 无订单号时不显示编号；retryHistory 支持 draft key 查找；专卖店变更箭头样式优化
6. 详情页规则文档：v0.4 → v0.5，新增 §6.10.13 完整章节

**构建验证**：主包 ~202 kB，TypeScript 类型检查通过

**定位**：订单生成历史规则的最终收口 —— 确保生成历史与审核历史分离、草稿删除原因正确承接、预占库存订单命名统一

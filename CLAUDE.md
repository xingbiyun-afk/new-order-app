...
> 适用于 Kimi、Codex、Trae、Workbuddy、Qoder 及其他参与本仓开发的 AI 工具。

1. 接手本仓任务时，默认先使用 GitNexus MCP 查询 `vue-reference/` 的页面、路由、类型、Pinia、Mock 和规则落点。
2. 与 Web 承接一致性相关的任务，默认同时对照：
   - `new-order-app/vue-reference/`
   - `../new-order-system`
3. `legacy/react-reference/` 仅作历史参考，除非任务明确要求回溯历史原型，否则默认降权。
4. GitNexus 负责结构理解和影响分析，业务口径仍以 `CURRENT_TECH_STACK.md`、本文件和 `docs/ai/项目协作基线.md` 为准。
...
| 日期 | 变更单 | 涉及页面 | 变更类型 | 变更摘要 | 影响验收口径 |
|------|--------|----------|----------|----------|-------------|
| 2026-07-16 | AI-20260716-001 | 公共文档/AGENTS.md | 补充 | 新增 GitNexus MCP 协作规则，统一 Kimi/Codex/Trae/Workbuddy/Qoder 默认先查 `vue-reference/`，历史 React 归档默认降权 | 否，协作基线增强 |
...
详细流程规则参考：`docs/product/work-order-budget/产品申请工单流程与状态规则.md`
...
- **详情页现行最终口径（CR-20260706-003 验收后，CR-20260707-002 订单状态术语统一后）**：
  ...
  - 详情页展示规则以《产品申请工单详情页面规则明细.md》（**v0.9 · CR-20260709-003**）为当前直接执行依据
  - 创建页展示规则以《产品申请工单创建页面规则明细.md》（**v0.3 · CR-20260709-002**）为当前直接执行依据
  - **文档治理（CR-20260710-001/002/003）**：产品文档已收口为分层阅读体系，统一入口《产品申请工单文档总览.md》；PRD 已瘦身为业务主线，页面规则下沉至对应规则明细文档；字段清单/页面结构说明回归各自定位；口径一致性复核完成，术语"冻结期预算号不可切换"统一替代"预算锁定"
...
**完成时间**：2026-07-10
**分支**：`feature/CR-20260710-003-doc-governance-review`（已合并 PR #53）

**主要变更**：
1. 7维度口径一致性复核（旧概念残留/固定口径/术语/金额/附件/职责边界/引用关系）
2. 发现1个口径偏差修正："预算锁定/预算号锁定" → "冻结期预算号不可切换"（4份文档7处：预算选择页规则明细 §1.2/§4.4/§8.2、PRD §7.4.1/更新日志、页面结构草图说明 §6.1/§6.2、AI开发任务说明 §2.5/§10更新日志）
3. DetailView.vue：移除 `getOrderStatusStyle()` 和 `getOrderBorderColor()` 中"已生成/生成失败"旧术语兼容分支
4. CreateView.vue：附件格式描述补全（+GIF、Word、PPT，对齐 PRD §5.3.5）
5. 补齐合并开发总结 + 项目日报

**定位**：文档治理收尾确认轮 —— 口径复核、术语统一、代码轻量巡检、协作文档补齐，不新增业务规则。
...
**完成时间**：2026-07-10
**分支**：`feature/CR-20260710-002-doc-optimize`（已合并 PR #52）

**主要变更**：
1. PRD 瘦身（v0.12→v0.13）：新增 §1.2 文档定位 + §1.3 配套文档表 + §1.4 推荐阅读顺序；§7/§15/§16 页面细则下沉引导
2. 预算核销规则 v0.1→v0.2：新增 §1 文档说明 + §2 预算核销生命周期 6 时点
3. 页面结构草图 v1.0→v1.1、字段清单 v0.5→v0.6：各自回归定位
4. 总览 v1.0→v1.1：升级为阅读导航文档
5. README 场景化阅读建议 + 统一入口引用

**定位**：第二轮文档治理 —— 重点文档合并收敛、结构瘦身与可读性完整性优化。
...
**完成时间**：2026-07-10
**分支**：`feature/CR-20260710-001-doc-governance`（已合并 PR #50 + PR #51 补修订）

**主要变更**：
1. 4份文档中文化改名：流程与状态规则、预算核销规则、我的页面规则明细、字段清单
2. 新建产品申请工单文档总览.md（分层入口+阅读顺序）
3. budget-management-integration.md 内容并入 PRD §23，原文档删除
4. open-questions.md 降级归档为待确认问题清单（历史归档）
5. 产品目录 README 重构为分层式入口
6. 全文交叉引用同步修正（29处旧文件名引用）
7. 补修订：PRD §23 加固、选择预算页规则 v0.2、AI 任务说明 v1.2

**定位**：第一轮文档治理 —— 命名中文化、目录主干收口、总览入口建立。

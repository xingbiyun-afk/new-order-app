...
| `/product-apply/detail/:id` | 详情页 | 驳回态Banner/基础信息/发起信息/预算与申请信息/产品明细(汇总+折叠)/附件/审批流/订单结果/审批操作/重新发起入口 |
...
1. **URL Search Params**：传递 `budgetId`、`storeCode`、`storeName`、`groupId`、`productCode`、`rejectedFrom`
   - `rejectedFrom`：驳回后重提场景的原工单号，存在时预算号锁定不可更换
...
13. **驳回后重新发起承接**（CR-20260703-001 §2）：
    - 冻结期原单重提：`?rejectedFrom=PAxxxxxx` 路由参数触发，`findBudgetByWorkOrderNo()` 找到原预算后 `store.selectBudget()` 带入并锁定（`goBudgetSelect` 拦截不可切换）
...
18. **详情页驳回态与重新发起**（CR-20260703-003 §4/§5）：
    - 已驳回状态首屏展示驳回 Banner（原因/驳回人/时间/备注）
    - 冻结期原单重提：`reapplyCondition.availability === 'freeze_period_locked'`，按钮文案"基于原单重新发起"
    - 非冻结期普通新建：`reapplyCondition.availability === 'non_freeze_new'`，按钮文案"重新发起"
    - 原预算已到期：`reapplyCondition.availability === 'budget_expired'`，按钮禁用+到期提示
    - 点击"基于原单重新发起"弹出确认弹窗，明确提示不允许切换预算号
19. **详情页产品明细汇总与折叠**（CR-20260703-003）：
    - 汇总行："共 X 笔产品申请订单，涉及 Y 个专卖店，共 Z 个产品"
